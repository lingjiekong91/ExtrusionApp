...
server<-function(input,output,session){
  ...
  output$mytable1 <- DT::renderDataTable({
    DT::datatable({
      ...
      if(input$PCSRD!="All"){
        data_PCS<-data_PCS[data_PCS$`Resin Description`==input$PCSRD,]
      }
      ...
      if(input$PCSTS_min!=PCSTSmin || input$PCSTS_max!=PCSTSmax){
        data_PCS<-data_PCS[data_PCS$`Tip Size (in)`>=input$PCSTS_min & data_PCS$`Tip Size (in)`<=input$PCSTS_max,]
      }
      if(input$PCSTLL!="All"){
